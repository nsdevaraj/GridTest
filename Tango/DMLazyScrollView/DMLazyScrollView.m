...
@synthesize numberOfPages,currentPage,prevPg,nextPg,movesForward;
@synthesize dataSource,controlDelegate,circularScrollEnabled,frontScrollEnabled,backScrollEnabled;
...
{
    return [self initWithFrameAndDirection:frame direction:DMLazyScrollViewDirectionVertical circularScroll:YES];
}
...
- (id)initWithFrameAndDirection:(CGRect)frame
    ...
    backScrollEnabled = YES;
    frontScrollEnabled = YES;
...
- (void) scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    BOOL canScrollBackward = ((circularScrollEnabled && backScrollEnabled) || (!circularScrollEnabled && self.currentPage != 0));
    // you can go forward if circular mode is enabled and current page is not the last page
    BOOL canScrollForward = ((circularScrollEnabled && frontScrollEnabled) || (!circularScrollEnabled && self.currentPage < (self.numberOfPages-1)));
